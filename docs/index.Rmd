---
title: 'GERAÇÃO DISTRIBUÍDA NO BRASIL E </br> SEU EFEITO MULTIPLICADOR'
...
- [**.white[GERAÇÃO DISTRIBUÍDA NO BRASIL]**](#mercado_fotovoltaico_brasil)


- [**.white[PRINCIPAIS ASPECTOS REGULATÓRIOS]**](#aspectos_regulatorios)


- [**.white[EFEITO MULTIPLICADOR (IMPACTO ECONÔMICO)]**](#efeito_multiplicador)
...
## DESAFIO DA PALESTRA
```{r desafio_do_tema}
# IMPORTANDO FIGURA DE DESAFIO: ----
knitr::include_graphics('nosso_curso.png')
```
...
- De maneira geral, o conceito de geração distribuída refere-se a um sistema de 
produção de energia elétrica em que múltiplas fontes de geração de energia são 
localizadas próximas aos consumidores finais. 


- Em oposição tem-se o modelo tradicional em que a geração ocorre em grandes 
...
- Contudo, como será discutido adiante, no Brasil a definição de geração 
distribuída ganha alguns contornos (limitações por regulação), tais como:

  - **Tipo de Fonte**: Renovável (hidráulica-pch, eólica, biomassa e cogeração 
  qualificada);
  - **Potência**: Apenas miniusinas (ou microusinas).
...
- Como se sabe, existem uma série de benefícios associados à utilização da fonte 
fotovoltaica:

  - Fonte inesgotável;
  - Reduzido consumo de água;
  - Reduzida necessidade de manutenção;
  - Reduzida necessidade de área para instalações (em comparação com demais 
  fontes);
  - Incentivo à pesquisa.
  
- Para além disso, no Brasil a fonte fotovoltaica possui elevada adaptabilidade
à geração descentralizada.
...
#   ggplot() +
#   geom_sf(mapping = aes(fill = media, frame = angulo,
#                             group = 1,
#     text = paste0('Irradiação: ', media, '(kWh/m2.dia)',
#                   '</br> Município)
#                         )) +
#   labs(fill = '')
#   theme_minimal() +
...
geração distribuída no Brasil, visto que, corresponde à **99,96%** do total de 
conexões à rede elétrica nesta modalidade, **98,94%** do total de consumidores 
atendidos e **98,91%** do total de potência nominal instalada:

  - Usinas: 2.117.490;

  - Unidades Consumidoras: 3.099.864;

  - Potência Instalada: 23.718.626,79 kWp.
...
- Existem diversas formas de atuação no mercado de geração distribuída no Brasil, 
cada uma apresentando diferentes desafios e, portanto, oportunidades.


- Porém, deve-se antes apresentar os principais aspectos da **RESOLUÇÃO NORMATIVA 
N° 482/2012** e de suas alterações, isto é, principalmente a **RESOLUÇÃO NORMATIVA Nº
...
- **Conceito de Choque Econômico**: Impactos expressam-se por meio da injeção de 
gastos adicionais de investimentos, por exemplo, compra e instalação de um sistema 
fotovoltaico (ou **pelos gastos adicionais de uma família ou empresa devido a 
economia obtida com a instalação**);

- Por sua vez, os efeitos deste choque na economia podem ser classificados em 
...
- **Efeito Direto**: incluem aquelas categorias econômicas diretamente afetadas 
pelas atividades mobilizadas pela variação no nível de consumo e investimento;

- **Efeito Indireto**: : impactos resultantes dos efeitos de encadeamento para 
trás das compras e vendas intersetoriais necessárias para atender à demanda final;

- **Efeito Induzido**: exprimem-se através das compras realizadas pelos 
consumidores, empregados direta e indiretamente, através das atividades econômicas 
beneficiadas pela variação dos investimentos ou consumos.
...
- A distribuição espacial desses efeitos multiplicadores, ou seja, a sua 
distribuição entre as economias das regiões e do país também podemser analisados 
pelo fluxo de comércio.

[DASHBOARD INICIAL](https://carneirogoulart.shinyapps.io/GD_App_MG/)
